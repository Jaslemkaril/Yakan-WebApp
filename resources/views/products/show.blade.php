        ...
        <div class="space-y-6">
            ...
            @php
                ...
                $availableStock = isset($initialAvailableStock)
                    ...
                    : (int) ($product->is_bundle ? $product->available_stock : ($product->inventory ? $product->inventory->quantity : $product->stock));
            @endphp
                    ...
                    <ul class="space-y-3">
                        @foreach($product->bundleItems as $bundleItem)
                            @php
                                ...
                                $componentImage = $component->image ?? null;
                                $isCloudinary = $componentImage && (str_starts_with($componentImage, 'http://') || str_starts_with($componentImage, 'https://'));
                                $imageSrc = $isCloudinary ? $componentImage : asset('uploads/products/' . $componentImage);
                            @endphp
                            <li class="flex items-center gap-3">
                                @if($componentImage)
                                    <img src="{{ $imageSrc }}" alt="{{ $component->name ?? 'Item' }}" 
                                ...
                                @endif
                                <div class="flex-1">
                                    <span class="text-sm font-medium text-gray-900">{{ $component->name ?? 'Item' }}</span>
                                </div>
                                <span class="text-sm font-semibold" style="color: #800000;">x{{ (int) $bundleItem->quantity }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
