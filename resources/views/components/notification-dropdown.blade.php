@auth
<div x-data="notificationDropdown()" x-cloak.duration.0="open" class="relative">
    <!-- Bell Icon Button -->
    ...
    <div x-show="open" x-cloak @click.away="open = false" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 -translate-y-2" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-3 w-96 bg-white rounded-2xl shadow-2xl z-50 overflow-hidden border border-gray-100 backdrop-blur-sm">
        ...
        <div class="bg-gradient-to-r from-maroon-600 via-maroon-700 to-maroon-800 p-6 text-white relative overflow-hidden">
            ...
            <div class="absolute inset-0 opacity-[0.04]" style="background-image: repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(255,255,255,1) 10px, rgba(255,255,255,1) 20px);"></div>
            
            <div class="flex items-center justify-between relative z-10">
                <div class="flex items-center gap-3">
                    <div class="w-11 h-11 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm border border-white/30 shadow-lg">
        ...
        <div class="max-h-96 overflow-y-auto scrollbar-premium">
            @forelse(auth()->user()->notifications()->orderBy('created_at', 'desc')->take(6)->get() as $index => $notification)
                <div onclick="markNotificationAsRead({{ $notification->id }})" class="p-4 border-b border-gray-100 hover:bg-gradient-to-r hover:from-maroon-50 hover:to-transparent transition duration-300 cursor-pointer group relative overflow-hidden {{ $notification->read_at ? 'opacity-60' : '' }}">
                    <!-- Left accent bar -->
