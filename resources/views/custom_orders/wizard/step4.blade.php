                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-2xl shadow-xl border-2 p-6" style="border-color:#e0b0b0;">
                        ...
                        @php
                            ...
                            $hasQueuedItems = count($batchItems) > 0;
                        @endphp
                        @if($hasQueuedItems)
                            {{-- Multiple items: show a picker modal --}}
                            <button type="button" onclick="openEditPickerModal()"
                            ...
                            {{-- Edit Picker Modal --}}
                            <div id="editPickerModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4" style="background-color:rgba(0,0,0,0.5);">
                                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                                    <div class="flex items-center justify-between mb-4">
                                        <h3 class="text-lg font-bold text-gray-900">Which item do you want to edit?</h3>
                                        <button type="button" onclick="closeEditPickerModal()" class="text-gray-400 hover:text-gray-600">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                            </svg>
                                        </button>
                                    </div>
                                    <p class="text-sm text-gray-500 mb-4">Select an item to go back and re-configure it. The other items will stay in your batch.</p>
                                    <div class="space-y-2 max-h-80 overflow-y-auto">
                                        {{-- Queued batch items --}}
                                        @foreach($batchItems as $bIdx => $bItem)
                                        @php
                                            $bEditWiz = $bItem['wizard_data'] ?? [];
                                            $bEditFabricType = '—';
                                            if (!empty($bEditWiz['fabric']['type'])) {
                                                $bEditFt = \App\Models\FabricType::find($bEditWiz['fabric']['type']);
                                                $bEditFabricType = $bEditFt ? $bEditFt->name : $bEditWiz['fabric']['type'];
                                            }
                                            $bEditPatternName = '—';
                                            if (!empty($bEditWiz['pattern']['selected_ids'])) {
                                                $bEditP = \App\Models\YakanPattern::find($bEditWiz['pattern']['selected_ids'][0]);
                                                $bEditPatternName = $bEditP ? $bEditP->name : '—';
                                            }
                                            $bEditMeters = $bEditWiz['fabric']['quantity_meters'] ?? null;
                                            $bEditQty = (int) ($bItem['form_data']['quantity'] ?? 1);
                                        @endphp
                                        <a href="{{ route('custom_orders.edit.batch.item', $bIdx) }}{{ $editAuthToken ? '?auth_token=' . $editAuthToken : '' }}"
                                            ...
                                            style="border-color:#e0b0b0; background-color:#fffafb;">
                                            <div class="w-7 h-7 rounded-full text-white text-xs font-bold flex items-center justify-center mr-3 flex-shrink-0" style="background-color:#800000;">{{ $bIdx + 1 }}</div>
                                            <div class="min-w-0">
                                                <p class="text-sm font-semibold text-gray-900">{{ $bItem['summary'] ?? 'Custom item ' . ($bIdx + 1) }}</p>
                                                <p class="text-xs text-gray-500">{{ $bEditFabricType }}{{ $bEditMeters ? ' · ' . $bEditMeters . 'm' : '' }} · Pattern: {{ $bEditPatternName }} · Qty {{ $bEditQty }}</p>
                                            </div>
                                    ...
                                    <button type="button" onclick="closeEditPickerModal()" class="mt-4 w-full px-4 py-2 rounded-xl border border-gray-300 text-gray-600 text-sm font-semibold hover:bg-gray-50 transition-all">
                                        Cancel
                                    </button>
                                </div>
                            </div>
                            ...
                            function openEditPickerModal() {
                                document.getElementById('editPickerModal').classList.remove('hidden');
                                document.body.style.overflow = 'hidden';
                            }
                            function closeEditPickerModal() {
                                document.getElementById('editPickerModal').classList.add('hidden');
                                document.body.style.overflow = '';
                            }
                            document.getElementById('editPickerModal').addEventListener('click', function(e) {
                                if (e.target === this) closeEditPickerModal();
                            });
                            </script>
                        @else
                            {{-- Only current item: go directly to step1 --}}
                            <a href="{{ $editUrl }}"
                                class="group flex items-center justify-center w-full px-8 py-3 rounded-xl font-bold transition-all duration-300 border-2 mb-3"
                                style="color:#800000; border-color:#800000; background-color:#fff5f5;"
                                onmouseover="this.style.backgroundColor='#fce8e8'"
