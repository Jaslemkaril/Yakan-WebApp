...
@php
    ...
    $bundleDiscountSelection = old('bundle_discount_type');
    $bundleCustomDiscount = old('custom_discount', '');

    if (!$bundleDiscountSelection) {
        $productDiscountType = strtolower((string) ($product->discount_type ?? ''));
        $productDiscountValue = (float) ($product->discount_value ?? 0);
        $presetPercentages = [10, 15, 20, 25, 30];

        if ($productDiscountType === 'percent' && in_array((int) round($productDiscountValue), $presetPercentages, true)) {
            $bundleDiscountSelection = 'percentage_' . (int) round($productDiscountValue);
        } elseif ($productDiscountType === 'fixed' && $productDiscountValue > 0) {
            $bundleDiscountSelection = 'custom';
            $bundleCustomDiscount = number_format($productDiscountValue, 2, '.', '');
        } else {
            $bundleDiscountSelection = 'percentage_10';
        }
    }
@endphp
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg border border-gray-200 p-4 sticky top-4">
                    ...
                    <input type="hidden" name="discount_type" id="bundleDiscountModeInput" value="{{ old('discount_type', $product->discount_type) }}">
                    <input type="hidden" name="discount_value" id="bundleDiscountValueInput" value="{{ old('discount_value', $product->discount_value) }}">
