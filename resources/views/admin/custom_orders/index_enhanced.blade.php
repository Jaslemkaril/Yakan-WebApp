        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50" id="ordersTableBody">
                    ...
                    {{-- ── Batch sibling rows (shown indented below the primary) ── --}}
                    @if($isBatchPrimary && !empty($order->batch_order_number))
                    @foreach(($batchSiblingsMap[$order->batch_order_number] ?? collect()) as $sibling)
                    @php
                        $siblingPs = $sibling->payment_status ?? 'unpaid';
                        $siblingPillClass = match($siblingPs) {
                            'paid'    => 'pill-paid',
                            'pending_verification' => 'pill-pending_verification',
                            'failed'  => 'pill-failed',
                            default   => 'pill-unpaid',
                        };
                        $siblingStatusLabel = ucfirst(str_replace('_',' ',$sibling->status ?? 'pending'));
                        $siblingPrice = $sibling->final_price ?? $sibling->estimated_price ?? 0;
                    @endphp
                    <tr class="bg-amber-50/40 border-l-4 border-amber-300" id="row-{{ $sibling->id }}">
                        <td class="pl-8 pr-4 py-2">
                            <div class="flex items-center gap-1.5">
                                <svg class="w-3 h-3 text-amber-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                                <span class="text-sm font-semibold text-gray-700">#{{ $sibling->id }}</span>
                                <span class="text-[10px] text-amber-700 font-medium">Batch Item</span>
                            </div>
                        </td>
                        <td class="px-4 py-2">
                            <p class="text-xs text-gray-500">{{ optional($sibling->user)->name ?? 'N/A' }}</p>
                        </td>
                        <td class="px-4 py-2">
                            @php
                                $sPatternModel = null;
                                if (!empty($sibling->design_metadata['pattern_id'])) {
                                    $sPatternModel = \App\Models\YakanPattern::find($sibling->design_metadata['pattern_id']);
                                }
                            @endphp
                            @if($sPatternModel && $sPatternModel->hasSvg())
                                <div class="w-8 h-8 rounded bg-gray-50 border border-gray-200 flex items-center justify-center p-0.5 overflow-hidden">
                                    {!! $sPatternModel->getSvgContent() !!}
                                </div>
                            @else
                        ...
                        <td class="px-4 py-2">
                            <p class="text-xs text-gray-600">{{ $sibling->fabric_type_name ?? '—' }}</p>
                        </td>
                        <td class="px-4 py-2">
                            <span class="text-sm font-bold text-gray-800">₱{{ number_format($siblingPrice, 2) }}</span>
                        </td>
                        <td class="px-4 py-2">
                            <span class="pill pill-{{ strtolower(str_replace([' ','_'],'-',$sibling->status ?? 'pending')) }} text-[10px]">{{ $siblingStatusLabel }}</span>
                        </td>
                        <td class="px-4 py-2">
                            <span class="pill {{ $siblingPillClass }} text-[10px]">{{ ucfirst(str_replace('_',' ',$siblingPs)) }}</span>
                        </td>
                        <td class="px-4 py-2">
                            <p class="text-xs text-gray-400">{{ $sibling->created_at->format('M d, Y') }}</p>
                        </td>
                        <td class="px-4 py-2 text-center">
                            <a href="{{ route('admin.custom-orders.show', $sibling->id) }}{{ request('auth_token') ? '?auth_token='.request('auth_token') : '' }}"
                               class="action-btn bg-gray-100 hover:bg-gray-200 text-gray-700 inline-flex" title="View Details">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                        </td>
                    </tr>
