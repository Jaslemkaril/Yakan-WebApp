...
<div class="space-y-6">
    <div class="bg-[#8b1d1d] rounded-2xl p-8 text-white shadow-xl">
        <h1 class="text-3xl font-bold mb-2">Refunded Orders</h1>
        <p class="text-red-100 text-lg">Review refunded orders and refund request information.</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-gray-600">Refund Date</th>
                    <th class="px-4 py-3 text-left font-semibold text-gray-600">Refund Reason</th>
            ...
            <tbody class="divide-y">
                @forelse($orders as $order)
                    @php
                        ...
                        $refundReason = $latestRefund->reason ?? ($order->admin_notes ?: 'N/A');
                        $refundStatus = $latestRefund ? ucfirst(str_replace('_', ' ', (string) $latestRefund->status)) : 'Refunded';
                    @endphp
                    <tr>
                        ...
                        <td class="px-4 py-3">{{ optional($order->updated_at)->format('M d, Y h:i A') }}</td>
                        <td class="px-4 py-3 max-w-xs truncate" title="{{ $refundReason }}">{{ $refundReason }}</td>
                        <td class="px-4 py-3">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-4 py-8 text-center text-gray-500">No refunded orders found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
