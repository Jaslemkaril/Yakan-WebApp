            ...
            $regularRefundRows = OrderRefundRequest::with(['order.user', 'user'])
                ...
                ->map(function (OrderRefundRequest $refundRequest) {
                    ...
                    $modalPayload = [
                        ...
                        'amount' => number_format((float) ($refundRequest->refund_amount ?? $refundRequest->approved_amount ?? $refundRequest->recommended_refund_amount ?? $order->total ?? $order->total_amount ?? ($order->subtotal + $order->shipping_fee) ?? 0), 2),
                    ...
                    return [
                        ...
                        'type' => 'refund',
                        'order_type' => 'normal',
                        'amount' => (float) ($refundRequest->refund_amount ?? $refundRequest->approved_amount ?? $refundRequest->recommended_refund_amount ?? $order->total ?? $order->total_amount ?? ($order->subtotal + $order->shipping_fee) ?? 0),
                        'status_key' => $normalizedStatus,
                        'status_label' => $statusLabel,
