...
<div class="chat-page-bg">
        ...
        <div class="info-card">
            ...
            @if($chatOrder)
                <a href="{{ route('admin.orders.show', $chatOrder->id) }}" class="btn-action btn-primary" style="margin-bottom: 10px; background: linear-gradient(135deg, #059669 0%, #047857 100%);">
                    <i class="fas fa-box"></i> View Order #{{ $chatOrder->order_ref }}
                </a>
            @endif
            @if($chat->user_id)
    ...
    @php
        ...
        $verifiedPayment = $chat->verifiedPayment();
    @endphp
    
    <!-- Send Payment Request - Only visible after customer accepts quote -->
    @if($hasAcceptedQuote)
    <div class="reply-card" style="background: linear-gradient(135deg, rgba(59, 130, 246, 0.05) 0%, rgba(96, 165, 250, 0.05) 100%); border-left: 4px solid #3b82f6;">
        <h3><i class="fas fa-credit-card"></i> Send Payment Request</h3>
        <form action="{{ route('admin.chats.payment.send', $chat) }}" method="POST">
            @csrf
            <div style="margin-bottom: 12px;">
                <label style="display: block; font-size: 0.875rem; font-weight: 600; color: #374151; margin-bottom: 8px;">Payment Amount (₱)</label>
                <input type="number" name="amount" step="0.01" min="0.01" required style="width: 100%; padding: 12px; border: 2px solid #3b82f6; border-radius: 8px; font-size: 0.875rem; color: #1f2937;" placeholder="Enter amount due">
                @error('amount')
                    <p style="color: #dc2626; font-size: 0.75rem; margin-top: 4px;">{{ $message }}</p>
                @enderror
            </div>
            <button type="submit" style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; padding: 12px 24px; border-radius: 8px; border: none; cursor: pointer; font-weight: 600; transition: all 0.3s; display: inline-flex; align-items: center; gap: 8px;">
    ...
    <!-- Payment Verification Section (if payment exists) -->
    @if($pendingPayment)
        <div class="reply-card" style="background: linear-gradient(135deg, rgba(34, 197, 94, 0.05) 0%, rgba(74, 222, 128, 0.05) 100%); border-left: 4px solid #22c55e;">
            <h3><i class="fas fa-check-circle"></i> Verify Payment Proof</h3>
            <div style="background: white; border: 1px solid #d1d5db; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
                <p style="color: #6b7280; font-size: 0.875rem; margin-bottom: 8px;"><strong>Payment ID:</strong> #{{ $pendingPayment->id }}</p>
                <p style="color: #6b7280; font-size: 0.875rem; margin-bottom: 8px;"><strong>Amount:</strong> ₱{{ number_format($pendingPayment->amount, 2) }}</p>
                <p style="color: #6b7280; font-size: 0.875rem; margin-bottom: 8px;"><strong>Method:</strong> {{ $pendingPayment->getPaymentMethodLabel() }}</p>
                <p style="color: #6b7280; font-size: 0.875rem;"><strong>Status:</strong> <span style="display: inline-block; padding: 4px 12px; background: #fef3c7; color: #92400e; border-radius: 20px; font-weight: 600;">{{ $pendingPayment->getStatusLabel() }}</span></p>
                
                @if($pendingPayment->payment_proof)
                    <div style="margin-top: 16px;">
                        <p style="color: #374151; font-weight: 600; margin-bottom: 8px;">Payment Proof:</p>
                        <img src="{{ asset($pendingPayment->payment_proof) }}" alt="Payment Proof" style="max-width: 100%; max-height: 300px; border-radius: 8px; border: 1px solid #d1d5db;">
                    </div>
                @endif
            </div>
            
            <form action="{{ route('admin.chats.payment.verify', $pendingPayment) }}" method="POST" style="display: flex; gap: 8px;">
                @csrf
                @method('PATCH')
                
                <div style="flex: 1;">
                    <textarea name="notes" style="width: 100%; padding: 12px; border: 2px solid #d1d5db; border-radius: 8px; font-size: 0.875rem; resize: vertical; background: white; color: #1f2937; min-height: 80px;" placeholder="Add notes (optional)..."></textarea>
                </div>
                
                <div style="display: flex; flex-direction: column; gap: 8px;">
                    <button type="submit" name="action" value="approve" style="background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: white; padding: 12px 20px; border-radius: 8px; border: none; cursor: pointer; font-weight: 600; transition: all 0.3s; display: inline-flex; align-items: center; gap: 6px; white-space: nowrap;">
                        <svg style="width: 16px; height: 16px;" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                        </svg>
                        Approve & Create Order
                    </button>
                    <button type="submit" name="action" value="reject" style="background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); color: white; padding: 12px 20px; border-radius: 8px; border: none; cursor: pointer; font-weight: 600; transition: all 0.3s; display: inline-flex; align-items: center; gap: 6px; white-space: nowrap;">
                        <svg style="width: 16px; height: 16px;" fill="currentColor" viewBox="0 0 20 20">
