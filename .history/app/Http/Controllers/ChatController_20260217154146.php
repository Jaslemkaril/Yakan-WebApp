...
class ChatController extends Controller
{
        ...
        if ($response === 'accepted') {
            ...
            try {
                $order = \App\Models\Order::create([
                    ...
                    'customer_phone' => $userAddress ? $userAddress->phone_number : (auth()->user()->phone ?? 'N/A'),
                ...
                \Log::error('Failed to create chat order', [
                    'error' => $e->getMessage(),
                    'chat_id' => $chat->id,
                    'user_id' => auth()->id(),
                ...
                return redirect()->route('chats.show', $chat)->with('error', 'Failed to create order. Please try again or contact support.');
            }
    ...
    public function setPaymentMethod(Request $request, $orderId)
    {
        ...
        $order->update([
            ...
            'status' => 'awaiting_payment' // Change status to show payment details instead of buttons
        ]);
