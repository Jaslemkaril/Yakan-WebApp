...
class ChatPaymentController extends Controller
{
    ...
    {
        // Check if user is admin - for now checking if session has admin flag
        if (!Auth::check() || Auth::user()->role !== 'admin') {
            abort(403, 'Only admin can send payment requests');
        }
    ...
    public function submitPaymentProof(Chat $chat, Request $request)
    {
        ...
        if ($request->hasFile('payment_proof')) {
            ...
            if (!is_dir(public_path('uploads/chat/payments'))) {
                mkdir(public_path('uploads/chat/payments'), 0755, true);
            }
            
            $file->move(public_path('uploads/chat/payments'), $filename);
